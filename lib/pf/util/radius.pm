...
sub perform_dynauth {
    ...
    my @controller_ip = split(',', $connection_info->{'nas_ip'});

    my @ip_addr;

    my $sock_in;

    #$connection_info->{'LocalAddr'},

    foreach my $ip (@controller_ip) {

        push @ip_addr, IO::Socket::INET->new(

#            LocalAddr => "10.201.0.138",
    ...
    $radius_request->show_unknown_entries(1);
    ...
    foreach $sock_in (@ip_addr) {
        $sock_in->send(auth_resp($radius_request->pack(), $connection_info->{'secret'}));
    ...
    while (1) {
        ...
        if (my @select_set = $select->can_read($connection_info->{'timeout'})) {
            ...
            foreach my $server (@select_set) {
                ...
                die("No answer from $connection_info->{'nas_ip'} on port $connection_info->{'nas_port'}")
                    ...
                    if (!$server->recv($rad_data, $MAX_TO_READ));
